...
int	ft_dollar_split_len(char *str)
{
	t_cmd			cmd;
	int				i;
	int				j;
...
char	*ft_dollar_split_fill(char *str)
{
	t_cmd			cmd;
	int				i;
	int				j;
...
int	ft_one_dollar_len(char *str)
{
	t_list	*env;
	int		i;
	...
	while (env)
	{
		if(!ft_strncmp(str, (char *)env->content, ft_strlen(str)))
		{
			i = 1;
			break ;
		}
	...
	if (i)
		return (ft_strlen((char *)env->content) - (ft_strlen(str) + 1));
	return (0);
}

int	ft_get_dollar_len(char *str)
{
	char	*str_split;
	char	**split;
	int		len;
...
int	ft_one_dollar_fill(char *str, char **str2, int *now)
{
	t_list	*env;
	int		i;
	int		j;
...
void	ft_fill_dollar(char *str, char **str2, int z, int *now)
{
	char	*str_split;
	char	**split;
	int		i;
	...
	if (split[z])
		ft_one_dollar_fill(split[z], str2, now);
	ft_free_split_2(&split);
	ft_free((void *)&str_split);
}
